...
defmodule SEO.JSONLD.HyperTocEntry do
  @moduledoc """
  A HyperToEntry is an item within a `SEO.JSONLD.HyperToc`, which represents a hypertext table of contents for complex media objects, such as `SEO.JSONLD.VideoObject`, `SEO.JSONLD.AudioObject`. The media object itself is indicated using `associatedMedia`. Each section of interest within that content can be described with a `SEO.JSONLD.HyperTocEntry`, with associated `startOffset` and `endOffset`. When several entries are all from the same file, `associatedMedia` is used on the overarching `SEO.JSONLD.HyperTocEntry`; if the content has been split into multiple files, they can be referenced using `associatedMedia` on each `SEO.JSONLD.HyperTocEntry`.

  Helper for building a Schema.org [HyperTocEntry](https://schema.org/HyperTocEntry) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:associated_media) => SEO.JSONLD.MediaObject.t() | map(),
            optional(:toc_continuation) => SEO.JSONLD.HyperTocEntry.t() | map(),
            optional(:utterances) => String.t(),
          ...
          | [
              ...
              toc_continuation: SEO.JSONLD.HyperTocEntry.t() | map(),
              utterances: String.t(),
  ...
  `"HyperTocEntry"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a HyperTocEntry JSON-LD map.

  ## Fields

  - `:associated_media` - A media object that encodes this CreativeWork. This property is a synonym for encoding.
  - `:toc_continuation` - A `SEO.JSONLD.HyperTocEntry` can have a `tocContinuation` indicated, which is another `SEO.JSONLD.HyperTocEntry` that would be the default next item to play or render.
  - `:utterances` - Text of an utterances (spoken words, lyrics etc.) that occurs at a certain section of a media object, represented as a `SEO.JSONLD.HyperTocEntry`.
  ...
  def build(attrs) do
    ...
    |> Map.put_new("@type", "HyperTocEntry")
  end
end
