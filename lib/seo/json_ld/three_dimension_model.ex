...
defmodule SEO.JSONLD.ThreeDimensionModel do
  @moduledoc """
  A 3D model represents some kind of 3D content, which may have `encoding`s in one or more `SEO.JSONLD.MediaObject`s. Many 3D formats are available (e.g. see [Wikipedia](https://en.wikipedia.org/wiki/Category:3D_graphics_file_formats)); specific encoding formats can be represented using the `encodingFormat` property applied to the relevant `SEO.JSONLD.MediaObject`. For the
  case of a single file published after Zip compression, the convention of appending '+zip' to the `encodingFormat` can be used. Geospatial, AR/VR, artistic/animation, gaming, engineering and scientific content can all be represented using `SEO.JSONLD.ThreeDimensionModel`.

  Helper for building a Schema.org [3DModel](https://schema.org/3DModel) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:is_resizable) => boolean(),
  ...
  `"3DModel"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a 3DModel JSON-LD map.

  ## Fields

  - `:is_resizable` - Whether the 3DModel allows resizing. For example, room layout applications often do not allow 3DModel elements to be resized to reflect reality.
