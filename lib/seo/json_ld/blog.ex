...
defmodule SEO.JSONLD.Blog do
  @moduledoc """
  A [blog](https://en.wikipedia.org/wiki/Blog), sometimes known as a "weblog". Note that the individual posts (`SEO.JSONLD.BlogPosting`s) in a `SEO.JSONLD.Blog` are often colloquially referred to by the same term.

  Helper for building a Schema.org [Blog](https://schema.org/Blog) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:blog_post) => SEO.JSONLD.BlogPosting.t() | map(),
            optional(:blog_posts) => SEO.JSONLD.BlogPosting.t() | map(),
          ...
          | [
              blog_post: SEO.JSONLD.BlogPosting.t() | map(),
              blog_posts: SEO.JSONLD.BlogPosting.t() | map(),
              issn: String.t(),
  ...
  `"Blog"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a Blog JSON-LD map.

  ## Fields

  - `:blog_post` - A posting that is part of this blog.
  - `:blog_posts` - Indicates a post that is part of a `SEO.JSONLD.Blog`. Note that historically, what we term a "Blog" was once known as a "weblog", and that what we term a "BlogPosting" is now often colloquially referred to as a "blog".
