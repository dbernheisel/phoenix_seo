...
defmodule SEO.JSONLD.Statement do
  @moduledoc """
  A statement about something, for example a fun or interesting fact. If known, the main entity this statement is about can be indicated using mainEntity. For more formal claims (e.g. in Fact Checking), consider using `SEO.JSONLD.Claim` instead. Use the `text` property to capture the text of the statement.

  Helper for building a Schema.org [Statement](https://schema.org/Statement) JSON-LD structure.
  """
  ...
  `"Statement"` respectively, plus any camelCased field keys
