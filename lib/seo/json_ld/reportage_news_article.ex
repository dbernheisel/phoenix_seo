...
defmodule SEO.JSONLD.ReportageNewsArticle do
  @moduledoc """
  The `SEO.JSONLD.ReportageNewsArticle` type is a subtype of `SEO.JSONLD.NewsArticle` representing
  news articles which are the result of journalistic news reporting conventions.

  In practice many news publishers produce a wide variety of article types, many of which might be considered a `SEO.JSONLD.NewsArticle` but not a `SEO.JSONLD.ReportageNewsArticle`. For example, opinion pieces, reviews, analysis, sponsored or satirical articles, or articles that combine several of these elements.

  The `SEO.JSONLD.ReportageNewsArticle` type is based on a stricter ideal for "news" as a work of journalism, with articles based on factual information either observed or verified by the author, or reported and verified from knowledgeable sources. This often includes perspectives from multiple viewpoints on a particular issue (distinguishing news reports from public relations or propaganda). News reports in the `SEO.JSONLD.ReportageNewsArticle` sense de-emphasize the opinion of the author, with commentary and value judgements typically expressed elsewhere.

  A `SEO.JSONLD.ReportageNewsArticle` which goes deeper into analysis can also be marked with an additional type of `SEO.JSONLD.AnalysisNewsArticle`.

  Helper for building a Schema.org [ReportageNewsArticle](https://schema.org/ReportageNewsArticle) JSON-LD structure.
  """
  ...
  `"ReportageNewsArticle"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a ReportageNewsArticle JSON-LD map.
  ...
  def build(attrs) do
    ...
    |> Map.put_new("@type", "ReportageNewsArticle")
  end
end
