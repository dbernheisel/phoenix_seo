...
defmodule SEO.JSONLD.Claim do
  @moduledoc """
  A `SEO.JSONLD.Claim` in Schema.org represents a specific, factually-oriented claim that could be the `itemReviewed` in a `SEO.JSONLD.ClaimReview`. The content of a claim can be summarized with the `text` property. Variations on well known claims can have their common identity indicated via `sameAs` links, and summarized with a `name`. Ideally, a `SEO.JSONLD.Claim` description includes enough contextual information to minimize the risk of ambiguity or inclarity. In practice, many claims are better understood in the context in which they appear or the interpretations provided by claim reviews.

  Beyond `SEO.JSONLD.ClaimReview`, the Claim type can be associated with related creative works - for example a `SEO.JSONLD.ScholarlyArticle` or `SEO.JSONLD.Question` might be `about` some `SEO.JSONLD.Claim`.

  At this time, Schema.org does not define any types of relationship between claims. This is a natural area for future exploration.

  Helper for building a Schema.org [Claim](https://schema.org/Claim) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:appearance) => SEO.JSONLD.CreativeWork.t() | map(),
            optional(:claim_interpreter) =>
              SEO.JSONLD.Organization.t() | map() | SEO.JSONLD.Person.t(),
            optional(:first_appearance) => SEO.JSONLD.CreativeWork.t() | map(),
          ...
          | [
              appearance: SEO.JSONLD.CreativeWork.t() | map(),
              claim_interpreter: SEO.JSONLD.Organization.t() | map() | SEO.JSONLD.Person.t(),
              first_appearance: SEO.JSONLD.CreativeWork.t() | map(),
  ...
  `"Claim"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a Claim JSON-LD map.

  ## Fields

  - `:appearance` - Indicates an occurrence of a `SEO.JSONLD.Claim` in some `SEO.JSONLD.CreativeWork`.
  - `:claim_interpreter` - For a `SEO.JSONLD.Claim` interpreted from `SEO.JSONLD.MediaObject` content, the `interpretedAsClaim` property can be used to indicate a claim contained, implied or refined from the content of a `SEO.JSONLD.MediaObject`.
  - `:first_appearance` - Indicates the first known occurrence of a `SEO.JSONLD.Claim` in some `SEO.JSONLD.CreativeWork`.
