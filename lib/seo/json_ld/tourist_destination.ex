...
defmodule SEO.JSONLD.TouristDestination do
  @moduledoc """
  A tourist destination. In principle any `SEO.JSONLD.Place` can be a `SEO.JSONLD.TouristDestination` from a `SEO.JSONLD.City`, Region or `SEO.JSONLD.Country` to an `SEO.JSONLD.AmusementPark` or `SEO.JSONLD.Hotel`. This Type can be used on its own to describe a general `SEO.JSONLD.TouristDestination`, or be used as an `additionalType` to add tourist relevant properties to any other `SEO.JSONLD.Place`. A `SEO.JSONLD.TouristDestination` is defined as a `SEO.JSONLD.Place` that contains, or is colocated with, one or more `SEO.JSONLD.TouristAttraction`s, often linked by a similar theme or interest to a particular `touristType`. The [UNWTO](http://www2.unwto.org/) defines Destination (main destination of a tourism trip) as the place visited that is central to the decision to take the trip.
  (See examples below.)

  Helper for building a Schema.org [TouristDestination](https://schema.org/TouristDestination) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:includes_attraction) => SEO.JSONLD.TouristAttraction.t() | map(),
          ...
          | [
              includes_attraction: SEO.JSONLD.TouristAttraction.t() | map(),
  ...
  `"TouristDestination"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a TouristDestination JSON-LD map.

  ## Fields

  - `:includes_attraction` - Attraction located at destination.
  ...
  def build(attrs) do
    ...
    |> Map.put_new("@type", "TouristDestination")
  end
end
