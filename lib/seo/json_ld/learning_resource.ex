...
defmodule SEO.JSONLD.LearningResource do
  @moduledoc """
  The LearningResource type can be used to indicate `SEO.JSONLD.CreativeWork`s (whether physical or digital) that have a particular and explicit orientation towards learning, education, skill acquisition, and other educational purposes.

  `SEO.JSONLD.LearningResource` is expected to be used as an addition to a primary type such as `SEO.JSONLD.Book`, `SEO.JSONLD.VideoObject`, `SEO.JSONLD.Product` etc.

  `SEO.JSONLD.EducationEvent` serves a similar purpose for event-like things (e.g. a `SEO.JSONLD.Trip`). A `SEO.JSONLD.LearningResource` may be created as a result of an `SEO.JSONLD.EducationEvent`, for example by recording one.

  Helper for building a Schema.org [LearningResource](https://schema.org/LearningResource) JSON-LD structure.
  """
  ...
  `"LearningResource"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a LearningResource JSON-LD map.
  ...
  - `:competency_required` - Knowledge, skill, ability or personal attribute that must be demonstrated by a person or other entity in order to do something such as earn an Educational Occupational Credential or understand a LearningResource.
  ...
  def build(attrs) do
    ...
    |> Map.put_new("@type", "LearningResource")
  end
end
