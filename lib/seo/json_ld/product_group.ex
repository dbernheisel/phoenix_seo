...
defmodule SEO.JSONLD.ProductGroup do
  @moduledoc """
  A ProductGroup represents a group of `SEO.JSONLD.Product`s that vary only in certain well-described ways, such as by `size`, `color`, `material` etc.

  While a ProductGroup itself is not directly offered for sale, the various varying products that it represents can be. The ProductGroup serves as a prototype or template, standing in for all of the products who have an `isVariantOf` relationship to it. As such, properties (including additional types) can be applied to the ProductGroup to represent characteristics shared by each of the (possibly very many) variants. Properties that reference a ProductGroup are not included in this mechanism; neither are the following specific properties `variesBy`, `hasVariant`, `url`.

  Helper for building a Schema.org [ProductGroup](https://schema.org/ProductGroup) JSON-LD structure.
  """

  @type attrs ::
          %{
            optional(:has_variant) => SEO.JSONLD.Product.t() | map(),
            optional(:product_group_id) => String.t(),
            optional(:varies_by) => SEO.JSONLD.DefinedTerm.t() | map() | String.t(),
          ...
          | [
              has_variant: SEO.JSONLD.Product.t() | map(),
              product_group_id: String.t(),
              varies_by: SEO.JSONLD.DefinedTerm.t() | map() | String.t(),
  ...
  `"ProductGroup"` respectively, plus any camelCased field keys
  ...
  @doc """
  Build a ProductGroup JSON-LD map.

  ## Fields

  - `:has_variant` - Indicates a `SEO.JSONLD.Product` that is a member of this `SEO.JSONLD.ProductGroup` (or `SEO.JSONLD.ProductModel`).
  - `:product_group_id` - Indicates a textual identifier for a ProductGroup.
  - `:varies_by` - Indicates the property or properties by which the variants in a `SEO.JSONLD.ProductGroup` vary, e.g. their size, color etc. Schema.org properties can be referenced by their short name e.g. "color"; terms defined elsewhere can be referenced with their URIs.
